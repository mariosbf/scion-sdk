...
use scion_stack::stack::ScionSocketBindError;
use sciparse::hummingbird::{Bandwidth, Reservation};
use thiserror::Error;
...
impl From<reqwest_connect_rpc::client::CrpcClientError> for HbirdRedemptionError {
    fn from(err: reqwest_connect_rpc::client::CrpcClientError) -> Self {
        HbirdRedemptionError::Transport(err.to_string())
    }
}

impl From<scion_connect_rpc::client::RequestError> for HbirdRedemptionError {
    fn from(err: scion_connect_rpc::client::RequestError) -> Self {
        HbirdRedemptionError::Transport(err.to_string())
    }
}
