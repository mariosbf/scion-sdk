...
use sciparse::{
    hummingbird::{Bandwidth, HbirdAuthKey, Reservation, ReservationInfo},
...
/// Encodes a reservation start time as the Unix timestamp the wire format carries.
///
/// Saturates at zero: a start time before the epoch is not a time any reservation can have, and
/// the service rejects it either way.
fn encode_start_time(start: SystemTime) -> u32 {
    start
...
impl From<RedemptionInfo> for v1::RedemptionInfo {
    fn from(info: RedemptionInfo) -> Self {
        v1::RedemptionInfo {
            ...
            start_time: encode_start_time(info.start_time),
...
fn from_proto_redemption_info(
    ...
    let start_time = ReservationInfo::decode_start(info.start_time);
...
/// into a [`sciparse::hummingbird::Reservation`].
pub fn from_proto_reservation(
    ...
    let auth_key: HbirdAuthKey = auth_key
        .try_into()
