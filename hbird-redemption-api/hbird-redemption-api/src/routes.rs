...
async fn redeem_handler(
    ...
    let (requests, client_key) = from_proto_requests(request).map_err(|e| {
        (axum::http::StatusCode::BAD_REQUEST, e.to_string()).into_response()
    })?;

    let reservations = service
        .redeem(requests, client_key)
        .await
        .map_err(|e| match e {
            ...
            other => {
                (axum::http::StatusCode::INTERNAL_SERVER_ERROR, other.to_string()).into_response()
            }
        })?;
...
async fn status_handler(
    ...
    service
        .status()
        .await
        .map(|info| ConnectRpc(StatusResponse { version: info.version }))
        .map_err(|e| {
